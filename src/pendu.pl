...
sub comparetab {
# me sert � voir si toutes les lettres ont �t� trouv�es
    my ($result,$i) = (0,0);
...
sub jeu {
	...
	{
	    # pour la condition : on boucle tant que $mot[$i] existe, c'est � dire jusqu'� la derni�re lettre !
...
@mot = split(//,uc($mot)); # on d�compose le mot dans un tableau de lettres
@mot_mystere = split(//, "*" x scalar(@mot));

print "Mot g�n�r� : @mot_mystere";
...
print "\nJe suis d�j� mort ! Dommage !" if $coups >= 15;
print "\nBravo ! vous avez gagn� !" if (comparetab());